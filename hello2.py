...
class Graph:
    ...
    def find_all_distances2(self, start_v):
        visited = []
    ...
    def find_all_distances(self, start_v):
        queue = [start_v]
        distances = [-1 for i in range(self.vertices)]
        distances[start_v] = 0
        visited = []
        while queue:
            cur_node = queue.pop()
            ...
            for i in range(self.vertices):
                if i not in visited and self.edges[cur_node][i] >= 6:
                    queue.append(i)
                    self.edges[cur_node][i] += distances[cur_node]
                    distances[i] = self.edges[cur_node][i]
        self.print_distances(distances)
